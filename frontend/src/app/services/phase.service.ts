...
export class PhaseService {
  ...
  getAllPhases(realEstateId?: number): Observable<PhaseListResponse> {
    ...
    return this.http.get<PhaseListResponse>(url)
  ...
  getPhasesByRealEstate(realEstateId: number): Observable<PhaseListResponse> {
    return this.http.get<PhaseListResponse>(`${this.apiUrl}/real-estate/${realEstateId}`)
      .pipe(catchError(this.handleError));
  }

  // Get phase by ID
  getPhaseById(id: number): Observable<PhaseSingleResponse> {
    return this.http.get<PhaseSingleResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

  // Create new phase
  createPhase(phase: Partial<Phase>): Observable<PhaseSingleResponse> {
    return this.http.post<PhaseSingleResponse>(this.apiUrl, phase)
      .pipe(catchError(this.handleError));
  }
  ...
  createPhaseForSelfRealEstate(phase: Omit<Partial<Phase>, 'realEstateId'>): Observable<PhaseSingleResponse> {
    return this.http.post<PhaseSingleResponse>(`${this.apiUrl}/self-real-estate`, phase)
      .pipe(catchError(this.handleError));
  }

  // Update phase
  updatePhase(id: number, phase: Partial<Phase>): Observable<PhaseSingleResponse> {
    return this.http.put<PhaseSingleResponse>(`${this.apiUrl}/${id}`, phase)
      .pipe(catchError(this.handleError));
  }
  ...
  deletePhase(id: number): Observable<PhaseResponse> {
    return this.http.delete<PhaseResponse>(`${this.apiUrl}/${id}`)
      .pipe(catchError(this.handleError));
  }

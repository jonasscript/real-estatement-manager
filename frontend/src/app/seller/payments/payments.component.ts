...
export class PaymentsComponent implements OnInit {
  ...
  selectedClient: SellerClient | null = null;
  ...
  receiptModalUrl: string | null = null;
  ...
  loadClients(): void {
    ...
    this.http.get<{ data: SellerClient[] }>(`${this.API_URL}/clients/assigned`, { headers })
      .subscribe({
  ...
  get filteredClients(): SellerClient[] {
  ...
  selectClient(client: SellerClient): void {
  ...
  sendInstallmentEmail(installment: Installment): void {
    ...
    this.paymentEmailService.sendInstallmentEmail(installment.id).subscribe({
      next: () => {
        ...
        this.emailMessage = `Correo enviado para la cuota #${installment.display_order ?? installment.installment_number}.`;
      },
      error: (error) => {
        this.sendingEmailInstallmentId = null;
  ...
  getClientInitials(client: SellerClient): string {
  ...
  getReceiptUrl(pay: Payment): string | null {
    return pay.proof_cloudinary_url || pay.proof_file_path || null;
  }

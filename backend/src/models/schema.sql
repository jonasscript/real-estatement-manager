...
-- Cron Configurations table - Periodicidad de tareas programadas por ciudadela (real_estate)
CREATE TABLE IF NOT EXISTS cron_configurations (
    ...
    day_of_week SMALLINT
        CHECK (day_of_week BETWEEN 0 AND 6), -- 0=domingo ... 6=sábado, requerido si frequency='weekly'
...
SELECT pg_temp.codex_sync_serial_sequence('cron_configurations');

SELECT pg_temp.codex_add_column_if_owner('cron_configurations', 'last_execution_at', 'ALTER TABLE public.cron_configurations ADD COLUMN last_execution_at TIMESTAMP');
SELECT pg_temp.codex_add_column_if_owner('cron_configurations', 'next_execution_at', 'ALTER TABLE public.cron_configurations ADD COLUMN next_execution_at TIMESTAMP');
...
-- Add Cron Configurations menu option for Real Estate Admin
INSERT INTO menu_options (name, label, path, icon, sort_order)
...
-- Assign Cron Configurations menu to real_estate_admin role
INSERT INTO role_menu_options (role_id, menu_option_id)
SELECT r.id, mo.id
FROM roles r,
  (SELECT DISTINCT ON (name) id, name FROM menu_options ORDER BY name, id) mo
...
-- Add Cron Configurations menu option for System Admin (manages any ciudadela)
INSERT INTO menu_options (name, label, path, icon, sort_order)
...
-- Assign Cron Configurations menu to system_admin role
INSERT INTO role_menu_options (role_id, menu_option_id)
SELECT r.id, mo.id
FROM roles r,
  (SELECT DISTINCT ON (name) id, name FROM menu_options ORDER BY name, id) mo

...
class PaymentEmailService {
  ...
  buildEmail(data) {
    ...
    const rawClientName = `${data.client_first_name} ${data.client_last_name}`.trim();
    const clientName = this.escapeHtml(rawClientName);
    ...
    const subject = `Recordatorio de pago - Cuota #${data.installment_number}`;
    const html = `
      <!doctype html>
      <html>
    ...
    const text = [
      `Hola ${rawClientName},`,
      `Te compartimos el detalle de tu cuota #${data.installment_number}.`,
      `Monto a pagar: ${amount}`,
      `Vencimiento: ${dueDate}`,
      `Estado: ${statusLabels[data.status] || data.status || 'Pendiente'}`,
      data.full_location ? `Propiedad: ${data.model_name || 'Propiedad'} - ${data.full_location}` : '',
      `Saludos, ${data.real_estate_name || 'la inmobiliaria'}`,
    ].filter(Boolean).join('\n');
  ...
  async sendInstallmentEmail(installmentId, senderUser) {
    ...
    if (!data.real_estate_id) {
      throw new Error('Real estate not available for this installment');
    }

    const email = this.buildEmail(data);
    const sentEmail = await sesEmailService.sendEmail({
      senderName: data.real_estate_name,
      senderEmail: data.ses_sender_email,
      senderDomain: data.ses_sender_domain,
      replyTo: data.real_estate_email,
    ...
    const insertResult = await query(
      `INSERT INTO payment_email_logs (
         installment_id, client_id, sent_by, recipient_email, subject,
         provider, sender_email, sender_domain, status, sent_at
       )
       VALUES ($1, $2, $3, $4, $5, 'ses', $6, $7, 'sent', CURRENT_TIMESTAMP)
       RETURNING *`,
      [
        data.id,
        data.client_id,
